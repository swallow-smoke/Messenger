...
function FBXScene({ url, onLoaded }: { url: string; onLoaded: () => void }): React.ReactElement {
...
class FBXErrorBoundary extends React.Component<BoundaryProps, { failed: boolean }> {
        ...
        <div className="inline-flex items-center gap-3 mt-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-xs text-white/70">
          ...
          <div className="flex-1">
            ...
            <div className="text-white/40 mt-0.5">
              {fileSize !== undefined ? `${(fileSize / 1024).toFixed(0)} KB · ` : ''}3D 모델 (FBX)
            </div>
          </div>
          <a
            href={fileUrl}
...
export function FBXModelPreview({ fileUrl, fileName, fileSize }: Props): React.ReactElement {
  ...
  return (
    <FBXErrorBoundary fileName={fileName} fileUrl={fileUrl} fileSize={fileSize}>
      <div
        className="mt-1 rounded-lg overflow-hidden border border-white/10 bg-black/30 relative group/fbx"
      ...
      >
        ...
        >
          ...
          <Suspense fallback={null}>
            <FBXScene url={fileUrl} onLoaded={handleLoaded} />
          </Suspense>
          <OrbitControls makeDefault />
        </Canvas>
        <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between opacity-0 group-hover/fbx:opacity-100 transition-opacity pointer-events-none">
          ...
          <a
            ...
            draggable
            onDragStart={(e) =>
              e.dataTransfer.setData('DownloadURL', `model/fbx:${fileName}:${fileUrl}`)
            }
            onClick={(e) => {
              e.preventDefault();
